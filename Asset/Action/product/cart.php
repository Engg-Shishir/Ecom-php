...
if (!isset($_SESSION['user_session'])) {
   ...
   if ($_POST['action'] == "addToCart") {
      ...
      if (mysqli_num_rows($rows) > 0) {
         ...
         $updateToCart11 = mysqli_query($conn, "UPDATE `preprocesscheckout` SET `cupon`='" . $data['discount'] . "' WHERE `uid`={$uid} ");
...
function writeCheckout($conn)
{
   ...
   $sql =  "SELECT cart.psno,cart.qty,cart.check,product.price,product.scharge FROM cart 
   INNER JOIN product ON cart.psno = product.sno
   where `check` = 1 AND cart.uid = {$uid} ";
   $row = mysqli_query($conn, $sql);
   ...
   if (mysqli_num_rows($row) > 0) {
      ...
      $sql1 =  "SELECT * FROM preprocesscheckout where `uid` = '" . $uid . "'  ";
      $row1 = mysqli_query($conn, $sql1);
      if (mysqli_num_rows($row1) > 0) {
         $rows = mysqli_fetch_assoc($row1);
         if ($rows['cupon'] == null) {
            $updateToCart11 = mysqli_query($conn, "UPDATE `preprocesscheckout` 
            ...
            // $updateToCart11 = mysqli_query($conn, "UPDATE `preprocesscheckout` SET `totalProduct`='" . $totalProduct . "',`subTotal`='" . $subTotal . "',`sCharge`='" . $sCharge . "',`total`='" . ($total - $rows['cupon']) . "' WHERE `uid`='" . $uid . "' ");
            
            $total = $total - $rows['cupon'];
            $updateToCart11 = mysqli_query($conn, "UPDATE `preprocesscheckout` 
            SET `totalProduct`={$totalProduct},
            `subTotal`={$subTotal},
            `sCharge`={$sCharge},
         ...
         if ($updateToCart11) {
            $insertChekoutTableDataget1 = mysqli_query($conn, "SELECT * FROM preprocesscheckout where `uid`='" . $uid . "' ");
            $data11 = mysqli_fetch_assoc($insertChekoutTableDataget1);
            return $data11;
         }
      } else {
         
            $_SESSION['checkoutSno'] = sernum();
   
         $sql = "INSERT INTO preprocesscheckout (`uid`,`totalProduct`,`subTotal`,`sCharge`,`total`,`sno`) VALUES ('" . $uid . "','" . $totalProduct . "','" . $subTotal . "','" . $sCharge . "','" . $total . "','" . $_SESSION['checkoutSno'] . "')";
         $insertChekoutTableData = mysqli_query($conn, $sql);
         if ($insertChekoutTableData) {

            $insertChekoutTableDataget = mysqli_query($conn, "SELECT * FROM preprocesscheckout where `uid` = {$uid} ");
            $data1 = mysqli_fetch_assoc($insertChekoutTableDataget);
            return $data1;
         }
      }
