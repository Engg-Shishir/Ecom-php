	...
	if($pname !="" && $pprice !="" && $pcategory !="Select Category" && $pdetails !="" && $pquantity !="" && $pdiscount !="" && $pscharge !="Select Shipping Charge"){
		...
		if($action == "update"){
			if(isset($_FILES['image'])){
				...
				if($image !=""){
					$run = mysqli_query($conn,"UPDATE `product` SET `name`='$pname',`price`='$pprice',`category`='$pcategory',`details`='$pdetails',`quantity`='$pquantity',`discount`='$pdiscount',`scharge`='$pscharge',`image`='$image' WHERE  `sno`='$sirial'");
					if($run){
						echo "success";
					}
				}
